...
.. caution::
    ...
    This syntax has been introduced in 2.9, it's still experimental.
    
 Twig Component came with a new syntax to ease the readability of your template:
 
 .. code-block:: html+twig

    {# templates/some_page.html.twig #}
    <t:table :headers="['key', 'value']" :data="[[1, 2], [3, 4]]">
        <t:block name="th_class">
            {{ parent() }}
        </t:block>
         <t:block name="td_class">
            {{ parent() }}
        </t:block>
        <t:block name="footer">
            <div class="data-table-footer">
 ...
 You can now use a custom HTML tag, prefix by t: and your component name.
 
 .. code-block:: html+twig
 
     <t:table></:table>
     // but also self closing tag
     <t:table/>
 
 You can pass attributes to your component like any HTML attribute and also use 
 the ':' prefix to your attribute to indicate that the value should be compiled by Twig.
 
 .. code-block:: html+twig
 
     // primary will be set to true
     <t:component primary></:component>
     <t:component text="hello" :user="user.id"/>
     // equal to
     <t:component text="hello" user="{{ user.id }}"/>
     // and pass object, or table, or anything you imagine
     <t:component :table="['col' => ['foo', 'oof']]"/>
